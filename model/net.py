...
from model.helper import ResnetBlock
...
class PyramidVisionTransformerImpr(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.input_downsample = nn.ModuleList([
            nn.Conv2d(mask_chans, embed_dims[i], kernel_size=1, stride=1, padding=0) for i in range(len(embed_dims))
        ])
        for conv in self.input_downsample:
    ...
    def forward_features(self, x, timesteps, cond_img):
        ...
        # stage 1
        x, H, W = self.patch_embed1(cond_img, None)
        x = x + self.input_downsample[0](F.interpolate(noisy_image, size=(H,W), mode='bilinear', align_corners=False)).flatten(2).transpose(1, 2)
        ...
        x = x + self.input_downsample[1](F.interpolate(noisy_image, size=(H,W), mode='bilinear', align_corners=False)).flatten(2).transpose(1, 2)
        x = torch.cat([time_token, x], dim=1)
        ...
        x = x + self.input_downsample[2](F.interpolate(noisy_image, size=(H,W), mode='bilinear', align_corners=False)).flatten(2).transpose(1, 2)
        x = torch.cat([time_token, x], dim=1)
        ...
        x = x + self.input_downsample[3](F.interpolate(noisy_image, size=(H,W), mode='bilinear', align_corners=False)).flatten(2).transpose(1, 2)
        x = torch.cat([time_token, x], dim=1)
...
class net(nn.Module):
    def __init__(self, num_classes, in_chans, **kwargs):
        super(net, self).__init__()
        self.in_chans = in_chans
        self.num_classes = num_classes
        self.backbone = pvt_v2_b4_m(in_chans=3, mask_chans=self.num_classes)
        self.decode_head = Decoder(dims=self.backbone.embed_dims, dim=256, num_classes=self.num_classes)
        self._init_weights()

    def _init_weights(self):
        pretrained_dict = torch.load(self._download_weights())
        model_dict = self.backbone.state_dict()

        self.input_fusion = torch.nn.Conv2d(self.in_chans, 3, kernel_size=1, stride=1, padding=0, bias=False)
        with torch.no_grad():
            self.input_fusion.weight[:] = 1 / self.in_chans
    ...
    def _download_weights(self):
        ...
        return hf_hub_download('Anonymity/pvt_pretrained', f'pvt_v2_b4_m.pth', cache_dir='./pretrained_weights')
    
    def forward(self, x, timesteps, cond_img):
        fused_cond_img = self.input_fusion(cond_img)
        features = self.backbone(x, timesteps, fused_cond_img)
